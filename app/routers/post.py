...
def createPost(msg: schemas.PostCreate, db: Session = Depends(get_db),
               current_user: dict = Depends(get_current_user)):  # adds data to different id
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    db.commit()
    return Response(
        status_code=status.HTTP_204_NO_CONTENT)  # for raise to func and display in postman we should use Response()


@router.put("/{id}", response_model=schemas.PostResponse)  # update func
def update_posts(id: int, post: schemas.PostCreate, db: Session = Depends(get_db), current_user: dict = Depends(
    get_current_user)):  # adds new data to the same id as prev whereas post adds data to different id
